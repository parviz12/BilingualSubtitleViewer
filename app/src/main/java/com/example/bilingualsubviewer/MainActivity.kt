...
class MainActivity : AppCompatActivity() {
    ...
    private fun loadMedia(uri:Uri){try{if(player==null){player=ExoPlayer.Builder(this).build().also{exo->playerView.player=exo;exo.addListener(object:Player.Listener{override fun onPlaybackStateChanged(s:Int){updatePlaybackControls()}})}};player?.setMediaItem(MediaItem.fromUri(uri));player?.setPlaybackSpeed(playbackSpeed);player?.prepare();player?.playWhenReady=true;fileNameText.text=uri.lastPathSegment?:getString(R.string.media_file);playerControls.visibility=View.VISIBLE;videoToggleButton.visibility=View.VISIBLE;speedButton.visibility=View.VISIBLE;loopButton.visibility=View.VISIBLE}catch(e:Exception){Toast.makeText(this,"Media error: ${e.message}",Toast.LENGTH_LONG).show()}}
