...
class MainActivity : AppCompatActivity() {
    ...
    private var singleSubtitleStartMs = -1L
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        ...
        when (event.keyCode) {
            ...
            KeyEvent.KEYCODE_DPAD_LEFT -> { p.seekTo((p.currentPosition - if (ctrl) 30000 else if (shift) 60000 else 5000).coerceAtLeast(0)); playSingleSubtitle = false; return true }
            KeyEvent.KEYCODE_DPAD_RIGHT -> { p.seekTo(p.currentPosition + if (ctrl) 30000 else if (shift) 60000 else 5000); playSingleSubtitle = false; return true }
            ...
            KeyEvent.KEYCODE_HOME -> { if (ctrl) selectSubtitle(currentPosition.coerceAtLeast(0), false) else previousSubtitle(); return true }
    ...
    private fun setupViews() {
        ...
        searchInput.addTextChangedListener(object : android.text.TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, st: Int, b: Int, c: Int) { searchQuery = s?.toString()?.trim() ?: ""; applySearch() }
        ...
        timelineSeekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            ...
            override fun onStopTrackingTouch(s: SeekBar) { player?.seekTo(s.progress.toLong()); cancelSingleSubtitle(); userSeeking = false }
        ...
        fileNameText.setOnClickListener {
            ...
            if (fullName.isNotEmpty() && fullName != getString(R.string.no_file)) AlertDialog.Builder(this).setTitle("Full file name").setMessage(fullName).setPositiveButton(android.R.string.ok, null).show()
        }
    }

    private fun setupInsets() {
        val root = findViewById<View>(R.id.rootContainer)
        ViewCompat.setOnApplyWindowInsetsListener(root) { v, i -> val b = i.getInsets(WindowInsetsCompat.Type.systemBars()); v.updatePadding(left = b.left, top = b.top, right = b.right, bottom = b.bottom); i }
    }
    ...
    private fun setupButtons() {
        ...
        navigationPlayPauseButton.setOnClickListener { togglePlayback() }
    }

    private fun togglePlayback() {
        player?.let {
            cancelSingleSubtitle()
    ...
    private fun loadMedia(uri: Uri) {
        try {
                ...
                player = ExoPlayer.Builder(this).build().also { exo ->
                    ...
                    exo.addListener(object : Player.Listener {
                        override fun onPlaybackStateChanged(s: Int) {
                            if (s == Player.STATE_ENDED) cancelSingleSubtitle()
                            updatePlaybackControls()
                        }
                    })
                }
            ...
            videoHidden = false
            playerView.visibility = View.VISIBLE; playerView.alpha = 1f; playerView.requestLayout()
            playerControls.visibility = View.VISIBLE
    ...
    private fun loadSubtitle(uri: Uri) {
        try {
            ...
            subtitles = parsed; currentPosition = 0; repeatCount = 0; cancelSingleSubtitle(); subtitleOffsetMs = 0; searchQuery = ""; searchInput.setText("")
            if (player == null) fileNameText.text = uri.lastPathSegment ?: getString(R.string.subtitle_file)
            subtitleAdapter.setAllItems(subtitles); applySearch(); syncSubtitleToPlayer()
        } catch (e: Exception) { Toast.makeText(this, "Error: ${e.message}", Toast.LENGTH_LONG).show() }
    }

    private fun applySearch() {
        val f = filteredSubtitles(); subtitleAdapter.setItems(f)
        if (searchQuery.isNotBlank() && f.isEmpty()) { emptyText.text = getString(R.string.no_search_results); emptyText.visibility = View.VISIBLE; return }
        emptyText.text = getString(R.string.open_subtitle_message); updateUi()
        if (searchQuery.isNotBlank() && f.isNotEmpty()) selectFirstSearchResult(f)
    }
    ...
    private fun selectFirstSearchResult(r: List<Subtitle>) {
        val f = r.firstOrNull() ?: return
        val i = subtitles.indexOfFirst { s -> s.index == f.index && s.startTime == f.startTime }
        if (i >= 0) selectSubtitle(i, false)
    }

    private fun selectSubtitle(i: Int, singlePlay: Boolean = false) {
        ...
        if (singlePlay) {
            singleSubtitleStartMs = s.startTime
            singleSubtitleEndMs = s.endTime
        } else {
            singleSubtitleStartMs = -1L
            singleSubtitleEndMs = -1L
        }
        player?.let { p ->
            p.setSeekParameters(SeekParameters.EXACT)
            p.playWhenReady = false
            p.seekTo((s.startTime - subtitleOffsetMs).coerceAtLeast(0L))
            p.playWhenReady = true
        }
        scrollToSubtitle(s); updateUi(); updatePlaybackControls()
    }

    private fun cancelSingleSubtitle() {
    ...
    private fun syncSubtitleToPlayer() {
        ...
        if (playSingleSubtitle && singleSubtitleStartMs >= 0L && singleSubtitleEndMs >= 0L) {
            if (time < singleSubtitleStartMs) return
            if (time >= singleSubtitleEndMs) {
                p.playWhenReady = false
                p.seekTo((singleSubtitleEndMs - subtitleOffsetMs).coerceAtLeast(0L))
                cancelSingleSubtitle()
        ...
        if (i >= 0 && time < subtitles[i].endTime) {
            if (i != currentPosition && !playSingleSubtitle) {
                currentPosition = i; updateUi(); scrollToSubtitle(subtitles[i])
            }
            if (loopSubtitle && i == currentPosition && time >= subtitles[i].endTime - 250 && !loopGuard) {
                loopGuard = true
                repeatCount++
                if (repeatCount >= repeatTarget) { repeatCount = 0; showStatus("🔁 Loop: 5/5 — continuing") }
                p.setSeekParameters(SeekParameters.EXACT)
                p.seekTo((subtitles[i].startTime - subtitleOffsetMs).coerceAtLeast(0L))
                p.playWhenReady = true
                handler.postDelayed({ loopGuard = false }, 350)
            }
        }
    }
    ...
    private fun toggleLoop() {
        ...
        loopSubtitle = !loopSubtitle; repeatCount = 0; repeatTarget = 5; updateLoopButton()
        showStatus(if (loopSubtitle) "🔁 Loop ON — groups of 5" else "⏹ Loop OFF")
    }
    ...
    private fun updatePlaybackControls() {
        val p = player ?: return
        val d = p.duration.coerceAtLeast(0); val pos = p.currentPosition.coerceAtLeast(0)
        if (!userSeeking) { timelineSeekBar.max = if (d > 0) d.coerceAtMost(Int.MAX_VALUE.toLong()).toInt() else 0; timelineSeekBar.progress = if (d > 0) pos.coerceAtMost(Int.MAX_VALUE.toLong()).toInt() else 0; currentTimeText.text = formatDuration(pos) }
        durationTimeText.text = formatDuration(d)
        val icon = if (p.isPlaying || p.playWhenReady) "⏸" else "▶"
        playPauseButton.text = icon; navigationPlayPauseButton.text = icon
    }
    ...
    private fun updateUi() {
        val h = subtitles.isNotEmpty() && currentPosition in subtitles.indices
        emptyText.visibility = if (h && (searchQuery.isBlank() || subtitleAdapter.itemCount > 0)) View.GONE else View.VISIBLE
        currentSubtitleText.visibility = View.GONE
        if (!h) { positionText.text = "0 / 0"; previousButton.isEnabled = false; nextButton.isEnabled = false; return }
        positionText.text = String.format(Locale.US,"%d / %d",currentPosition+1,subtitles.size)
        previousButton.isEnabled = currentPosition > 0; nextButton.isEnabled = currentPosition < subtitles.lastIndex
        subtitleAdapter.setCurrentIndex(subtitles[currentPosition].index)
    }

    private class SubtitleAdapter(private var items: List<Subtitle>, private var allItems: List<Subtitle>, private val onClick: (Int) -> Unit) : RecyclerView.Adapter<SubtitleAdapter.ViewHolder>() {
        private var currentIndex: Int? = null
        fun setAllItems(x: List<Subtitle>) { allItems = x }
        fun setItems(x: List<Subtitle>) { items = x; notifyDataSetChanged() }
        fun setCurrentIndex(x: Int) { currentIndex = x; notifyDataSetChanged() }
        fun visiblePositionOf(s: Subtitle) = items.indexOfFirst { it.index == s.index && it.startTime == s.startTime }
        override fun onCreateViewHolder(p: android.view.ViewGroup, t: Int) = ViewHolder(android.view.LayoutInflater.from(p.context).inflate(R.layout.item_subtitle,p,false))
        override fun onBindViewHolder(h: ViewHolder, p: Int) {
            val s = items[p]; val active = s.index == currentIndex
            h.number.text = s.index.toString(); h.text.text = BidiUtils.format(s.text); h.time.text = formatTime(s.startTime)
            h.itemView.alpha = if (active) 1f else .75f
            h.itemView.setBackgroundResource(if (active) R.drawable.subtitle_current_background else android.R.color.transparent)
            h.text.setTypeface(null, if (active) Typeface.BOLD else Typeface.NORMAL); h.number.setTypeface(null, Typeface.BOLD)
            h.itemView.setOnClickListener { val i = allItems.indexOfFirst { it.index == s.index && it.startTime == s.startTime }; if (i >= 0) onClick(i) }
        }
        override fun getItemCount() = items.size
        private fun formatTime(ms: Long): String { val t = ms / 1000; return String.format(Locale.US,"%02d:%02d",t/60,t%60) }
        class ViewHolder(v: View) : RecyclerView.ViewHolder(v) { val number: TextView = v.findViewById(R.id.itemNumber); val text: TextView = v.findViewById(R.id.itemText); val time: TextView = v.findViewById(R.id.itemTime) }
    }
}
